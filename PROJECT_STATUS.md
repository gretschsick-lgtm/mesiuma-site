...
| 項目 | 値 |
|------|-----|
| store_handles追加前 | 3000件 (committed) |
| store_handles追加後 | **3162件** |
| 追加件数 | **+162件** |
| complete_info件数 | 793件 |
| slot件数 | 541件 |
| pachinko件数 | 252件 |
| unknown件数 | **0件 ✅** |
| 重複x_url | 0件 ✅ |
| machine_id=NULL | 0件 ✅ |
| store_id=NULL | 36件 (Supabase未登録) |

#### complete_info品質修正

| 修正内容 | 件数 | 詳細 |
|---------|------|------|
| store名修正 | 2件 | `楽園池袋` → `楽園池袋店ゲートウェイ` (rakuenikebukuro, 2026-05-27) |
| ランキング再生成 | 1回 | 修正反映後に update_ranking() 実行 |
...
#### store_handles追加内訳 (主なチェーン/地域)

前回コミット(3000件)から今回(3162件)までの+162件:
- メガコンコルド（愛知）: 7件（megacon1111/megab1111/1177oguchi/mega_mktp/megacon777/megaconkariya/conpachi800等）
- DSG（石川/金沢）: 3件（dsg45191683/mgr_k_dsg/musclekdsg）
- 宮城（パラディソ）: 5件
- 岩手（ジャムフレンド）: 4件
- 新潟（ミッドガーデン）: 3件
- その他全国: 多数
...
| 項目 | 値 |
|------|-----|
| 追加前 | 2976件 (store:2745, manager:231) |
| 追加後 | **3000件** (store:2761, manager:239) |
| 追加件数 | **+24件** (store:+16, manager:+8) |
| 重複除外 | 0件 |
| **3000件達成** | ✅ |
| 5000件まで残 | **2000件** |
...
| 地域 | 追加件数 | 主な追加店舗 |
|------|---------|------------|
| 北海道 | +7 | マルハン函館港、マルハン江別、ハッピー小樽駅前、豊平ひまわりスタッフ3件、マルハン中標津 |
| 福島県 | +6 | ビックつばめ会津若松、マルハン矢吹、アラジン須賀川・八山田・白河、マルハン福島エリア |
| 静岡県 | +5 | コンコルド静岡紺屋町・浜松雄踏・掛川・緑が丘、マルハンメガシティ三島駿東 |
| 山梨県 | +4 | ABC甲府上阿原・甲府アルプス通り・甲府昭和通り、ゲームパニック甲府 |
| 新潟県 | +1 | 朱鷺ゆかり@新潟マルハン |
| 香川県 | +1 | えの木高松店 |

---

### 地方ホール発掘フェーズ続き (2026-06-12) ✅

#### 結果サマリ

| 項目 | 値 |
|------|-----|
| 追加前 | 2902件 (store:2678, manager:224) |
| 追加後 | **2976件** (store:2745, manager:231) |
| 追加件数 | **+74件** (store:+67, manager:+7) |
| 重複除外 | 5件（store名重複） |
| 5000件まで残 | **2024件** |

#### 対象地域（13地域）

佐賀・奈良・和歌山・群馬・新潟・静岡・山梨・滋賀・徳島・高知・愛媛・香川・鹿児島

#### 地域別追加件数（ターゲット13地域）

| 地域 | 追加件数 | 主な追加店舗 |
|------|---------|------------|
| 鹿児島県 | +10 | テンガイ戸島、マルハン鹿屋・卸本町、まるみつ鹿屋・霧島、MGM出水、モリナガ系3件 |
| 群馬県 | +9 | D'ステーション太田矢島・前橋若宮・金古、ビックマーチ伊勢崎、マルハン前橋天川・群馬エリア等 |
| 和歌山県 | +5 | メトロヒルズ、ウイング橋本、LOVE海南、アクア、マルハン和歌山インター |
| 滋賀県 | +5 | ICHI-BAN近江八幡・本店、Durham S-1、ベニス栗東、キング観光長浜インター |
| 奈良県 | +4 | ARROW天理、123+N橿原、サイバー大和小泉、ベニス奈良 |
| 佐賀県 | +3 | フェイス701巨勢、テンガイ唐津、キャロル96上峰 |
| 徳島県 | +3 | ミリオン中吉野・平和台、123松茂 |
| 愛媛県 | +3 | MGキスケ、KIT(キスケBOX3F)、Super Nikko |
| 新潟県 | +2 | マルハン長岡平島、アピナ長岡 |
| 静岡県 | +1 | SKY CONCORDE富士宮 |
| 山梨県 | +1 | ニューアサヒ甲府南 |
| 香川県 | +1 | マルハン高松 |
| 高知県 | +1 | マルハン高知高須 |
...
| 地域 | 追加件数 | 主な追加店舗 |
|------|---------|------------|
| 千葉県 | +11 | キクヤ千葉、エクスアリーナ柏、SUPERWAVE柏、ウエスタン佐倉等 |
| 茨城県 | +8 | レイト取手、MGM水戸北スタッフ、ビックマーチ系4件等 |
| 栃木県 | +7 | ライブガーデン系4件、アイランド佐野、スーパーBOSS等 |

#### 重複除外 5件

| ハンドル | 理由 |
|---------|------|
| shotaro11337799 | マルハンメガシティ水戸赤塚（maruhanakatsuka と store名重複） |
| maruhanmegacity | マルハンメガシティ静岡（narichanmaruhan と store名重複） |
| otashindocho | マルハン太田新道町店（gunnma7777 と store名重複） |
| satotenc | マルハン甲府店（tamaki_____a と store名重複） |
| mgmsakai | MGM境店（MgmSakai と store名重複） |

---

### 全国地方ホール発掘フェーズ (2026-06-11) ✅

#### 結果サマリ

| 項目 | 値 |
|------|-----|
| 追加前 | 2859件 (store:2639, manager:220) |
| 追加後 | **2902件** (store:2678, manager:224) |
| 追加件数 | **+43件** (store:+39, manager:+4) |
| 重複除外 | 4件（store名重複） |
| 5000件まで残 | **2098件** |

#### 対象地域（21地域）

岡山・広島・茨城・群馬・埼玉・新潟・長野・岐阜・三重・石川・富山・福井・山形・秋田・青森・岩手・宮崎・大分・佐賀・長崎・鹿児島

#### 地域別追加件数

| 地域 | 追加件数 | 主な追加店舗 |
|------|---------|------------|
| 三重県 | +6 | ウイング津、MGM四日市、キング観光桑名、ホームラン系2件、MGMグループ長 |
| 宮崎県 | +5 | Dステーション都城・延岡、がちゃぽん都城、マルハン都城2件 |
| 岩手県 | +5 | マルハン北上、カネマン系4件（大船渡・津志田・陸前高田・奥州） |
| 岐阜県 | +4 | キクヤ島、ベガビック1200、ジョイランド、コンコルド777羽島 |
| 石川県 | +3 | DSG MEGA WORLD金沢、イーゾーン金沢（公式・統括） |
| 福井県 | +3 | DSGメガワールド鯖江、DSGアリーナ春江、DSG福井エリア長 |
| 長野県 | +3 | チャンピオンBelz、ニュートーキョー若穂・篠ノ井 |
| 埼玉県 | +2 | コトブキプラスさいたま新都心、パチンコキサイ |
| 富山県 | +2 | ノースランド、スーパーUSA富山（店長） |
| 山形県 | +2 | SLOT ZERO山形、ベガスベガス鉄砲町 |
| 秋田県 | +2 | マルハン由利本荘、ニューヨーク広面 |
| 長崎県 | +2 | Dステーション松浦・佐々インター |
| 岡山県 | +1 | スーパーハリウッド800 |
| 広島県 | +1 | スロットLL |
| 大分県 | +1 | Dee0.GOGO |
| 青森県 | +1 | ダイトー五所川原 |
| 群馬県 | 0 | 新規未発見 |
| 新潟県 | 0 | 新規未発見 |
| 茨城県 | 0 | bm_higashi・shotaro11337799は store名重複除外 |
| 佐賀県 | 0 | 新規未発見 |
| 鹿児島県 | 0 | 新規未発見 |
...
| ハンドル | 理由 |
|---------|------|
| bm_higashi | ビックマーチ河和田東店（store名重複） |
| shotaro11337799 | マルハンメガシティ水戸赤塚（store名重複） |
| umg1013 | カネマン盛岡南777（store名重複） |
| keizkanazawa | KEIZラパーク金沢店（store名重複） |

#### 備考

- 第1次エージェント6本はAPIレート制限で結果なし。第2次エージェント5本で回収完了。
- 群馬・新潟・佐賀・鹿児島は既存登録済み or 公式X未発見。
...
| 指標 | 変更前 | 変更後 |
|------|--------|--------|
| COMPLETE_QUERIES総件数 | 299件 | **227件** |
| 削除件数 | - | **80件** |
| 追加件数 | - | **+8件** |
| 取得0件クエリ（通常） | 97件 | **18件**（チェーン保留） |
| 取得5件以上クエリ | - | **139件** |
| 実データカバレッジ | 680/735件 | **702/735件** (+22件) |
| 重複クエリ | - | **0件** |
...
| カテゴリ | 件数 | 理由 |
|---------|------|------|
| 完走系 | 6件 | 実データ0件（スロット用語だが投稿なし） |
| COMPLETE英語系 | 2件 | 実データ0件（英語表記使用なし） |
| 枚数系 | 4件 | 実データ0件（16000/19000/20000/22000枚） |
| 全台系/同時系 | 4件 | 異なる概念・実データ0件 |
| スマスロ完走系 | 3件 | 実データ0件 |
| スマスロコンプリート（スペースなし） | 1件 | 実データ0件 |
| 古い機種・0件機種 | 12件 | バジリスク/Re:ゼロ/転スラ等 |
| スペースなし版（スペースあり版と重複） | 11件 | スマスロゴッドイーター/バイオRE:3等 |
| L系0件（廃盤・稀少） | 11件 | L吉宗/L鉄拳6/L無職転生等 |
| 機種名機能発動重複 | 9件 | 番台版で十分カバー済み |
| チェーン0件（機能発動重複） | 11件 | ガーデン/楽園/フェイス等 |
| ARROW/D'station等 | 6件 | 実データ0件 |

#### 追加クエリ（8件）- 実データから確認

| クエリ | 実データ件数 | 根拠 |
|--------|------------|------|
| `コンプリート機能が発動` | 30件 | 「が」付きフォームが実際に多い |
| `コンプリート機能が作動` | 21件 | 「が作動」表現が実際に使われる |
| `コンプリート報告` | 45件 | 番台なし報告投稿を補足 |
| `遊技台停止 番台` | 29件 | キャッスル熱田等の定型文 |
| `スマスロ バイオハザードRE:3 コンプリート` | 5件 | スペースあり版（スペースなし版は0件） |
| `Lパチスロ コンプリート 番台` | 16件 | 「Lパチスロ」プレフィックスを補足 |
| `コンプリートのご報告` | 5件 | 楽園系の丁寧語表現 |
| `コンプリートしました 番台` | 7件 | 「しました」形式を補足 |

#### 残存0件クエリ（18件）- チェーン系保留

メガガイア/コンコルド/メッセ/エスパス/ワンダーランド/クリエ/ジャパン/ニューアルファ/アビバ/ニラク/ZENT/スタジアム/ビックマーチ/コンコルド系 → 将来の新規投稿検出のため保留
...
#### store_handles 2608 → 2839件 (+231件) ✅

| フェーズ | 追加件数 | 内訳 |
|---------|---------|------|
| events_public.json 高信頼ハンドル抽出 | +16件 | チェーン系高信頼ハンドル |
| 4エージェント並列（チェーン・地域） | +98件 | 楽園/ガーデン/VV/コンコルド/ビックマーチ/D'station/MGM/KEIZ/NIKKO/北海道東北北陸/四国中国九州 |
| 福岡県・宮城県 個人店 | +20件 | パラディソ7件、ソユー4件、メガフェイス2件等 |
| ガイア/ガイアネクスト追加 | +15件 | ミクちゃんガイア兵庫9件、東京/秋田/静岡/広島等 |
| マルハン残店舗 | +10件 | 愛知/長野/岡山/香川/神奈川/埼玉/東京 |
| キコーナ/ピーアーク/アミューズ | +25件 | キコーナ24件（茨城/埼玉/神奈川/大阪/兵庫/奈良/和歌山/京都）、ピーアーク1件 |
| 大阪府・兵庫県 個人店 | +30件 | ベラジオ7件、マジックバード4件、ヴィーナスギャラリー3件等 |
| 東京都・神奈川県 個人店 | +33件 | ヒノマル12件、グランパ/グランドホール系5件、パールショップともえ3件等 |
| **合計** | **+231件** | **2608件 → 2839件** |

**重複除外数:** 80件以上（既登録ハンドルのスキップ）
